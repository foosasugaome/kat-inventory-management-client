...
export default function TransactionForm ({
  ...
  const handleSubmit = e => {
    ...
    try {
      axios
        .put(
          `${process.env.REACT_APP_SERVER_URL}/api-v1/inventory/${invId}/transaction`,
          form
        )
        .then(response => {
        ...
        <div className='form-container'>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <input
              type='number'
              id='transCount'
              value={form.transCount}
              onChange={e => setForm({ ...form, transCount: e.target.value })}
              required
            />
            ...
            <input
              ...
              id='transPPU'
              value={form.transPPU}
              onChange={e => setForm({ ...form, transPPU: e.target.value })}
              required
            />
            <label htmlFor='transNotes'>Note </label>
            <textarea
              id='transNotes'
              value={form.transNotes}
              onChange={e => setForm({ ...form, transNotes: e.target.value })}
            />
            <label htmlFor='transOwner'>Created by</label>
            <input
              type='text'
              id='transOwner'
              value={form.transOwner}
              disabled
            />
            <p>
